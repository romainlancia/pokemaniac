...
const HomeBody = () => {
    ...
    () => [
      {
        ...
        header: '#',
        cell: ({ getValue }) => {
          const [name, id, sprites] = getValue<Array<string>>();
            ...
            <div className="flex flex-row items-center justify-center">
              {spritesParsed?.front_default ? (
                <Image
                  ...
                  alt={name}
                  src={spritesParsed.front_default}
                  width={56}
                  height={42}
                />
              ) : (
      ...
      {
        accessorFn: ({ pokemon }) =>
          pokemon.stats.reduce((acc, value) => acc + value.baseStat, 0),
        header: 'Total',
        cell: ({ getValue }) => (
          <p className="font-bold">{getValue<number>()}</p>
        ),
      },
      {
  ...
  return data ? (
    <main>
      <div
        className="relative top-0 w-full h-28"
      ...
      <div
        className="rounded p-4 mx-8 mb-4"
        style={{ backgroundColor: '#F0F0F0' }}
      >
        <p className="mb-4">
          This is a full list of <em>every Pokémon</em> from all 8 generations
          of the Pokémon series, along with their main stats.
        </p>
        <p>
                  ...
                  return (
                    <tr key={row.id}>
                      {row.getVisibleCells().map((cell) => (
                        <td
                          key={cell.id}
                          className="font-sans text-center px-3 py-1 h-16 border-b"
                        >
                          {flexRender(
